...
public class ColorTetrisDriver extends Applet {
   ...
   private class ButtonActionListener implements ActionListener {
      public void actionPerformed(ActionEvent event) {
         ...
         // left swap button was clicked
            ...
            // swap tops of this and left stack (EVEN if left stack is empty!)       LEFT SWAP
            // YOUR CODE HERE
            if (colMarker == 0){
			}else if(stacks[colMarker-1].height() == 0){
				stacks[colMarker-1].push(stacks[colMarker].peek());
            	stacks[colMarker].pop();
				}else{
					Object[] temp = new Object[numCols];
					temp[1] = stacks[colMarker].peek();
					stacks[colMarker].push(stacks[colMarker-1].peek());
					stacks[colMarker-1].push(temp[1]);  	
			}
         }
         
         // right swap button was clicked                                           RIGHT SWAP
   ...
   public void removeCombos() {
      // check for 3 in a row horizontally and remove them                      CHECK FOR 3 HORIZ
      // YOUR CODE HERE
       /*for (int j=0; j<numCols-2; j++) {
            if (stacks[j].peek().equals(stacks[j+1].peek())) {
					if(stacks[j].peek().equals(stacks[j+2].peek())){
						stacks[j].pop();
						stacks[j+1].pop();
						stacks[j+2].pop();
					}
         ...
         if (done == false) {
            ...
            for (int j=0; j<cols; j++) {
               if (stacks[j].height()==0) {}
                  ...
                  // YOUR CODE HERE to draw the whole stack                  DRAW THE STACK
      				Stack colClone[] = new Stack[numCols];
      				colClone[j] = stacks[j];
                  		stackColor = (Integer) stacks[j].peek();
						setBlockColor(g, stackColor);
                  // first row is 2 rows down from top of window
						while( colClone[j] != null){
							if(colClone[j].hasNext()){
		                  		stackColor = (Integer) stacks[j].next();
								setBlockColor(g, stackColor);
							}
						}		
